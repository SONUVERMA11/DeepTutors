...
export default function LoginPage() {
  ...
  const handleSendOTP = (e: React.FormEvent) => {
    ...
    if (phoneNumber.length > 5) {
      setOtpSent(true);
      // In production, this would trigger WhatsApp OTP via OTPLESS
    }
  };
  ...
  const handleVerifyOTP = (e: React.FormEvent) => {
    e.preventDefault();
    if (otp.length === 6) {
      // Mocking successful login routing to student dashboard
      ...
      {!otpSent ? (
        <form onSubmit={handleSendOTP} className={styles.form}>
          <div className="input-group">
            <label htmlFor="phone" className="input-label">WhatsApp Number</label>
            ...
            <p className="text-caption" style={{ marginTop: "4px" }}>
              We&apos;ll send you an OTP via WhatsApp. Zero SMS fees!
            </p>
          </div>
